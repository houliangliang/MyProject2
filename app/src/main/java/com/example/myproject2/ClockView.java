...
public class ClockView extends View {
    ...
    private void init(Context context, AttributeSet attars) {
        ...
        mBorderColor = array.getColor
                (R.styleable.clock_view_clock_border_color, Color.parseColor("#000000"));
        ...
        clockBackColor = array.getColor(R.styleable.clock_view_clock_back_color, Color.parseColor("#ffffff"));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        mFormat1 = format.format(date);


        mV = mBounds.centerX() - mBounds.centerX() / 2;
        mV1 = mBounds.centerY() - (mBounds.centerY()*0.15f);
        canvas.drawText(mFormat1, mV, mV1, p);
        canvas.drawRect(new Rect((int) mV -5,(int) mV1 -35,(int) mV +65,(int) mV1+15), p);
        canvas.drawCircle(mBounds.centerX(), mBounds.centerY(), radius, p);//绘制表盘
        //是否绘制边界矩形
...
//        }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (event.getAction() == MotionEvent.ACTION_DOWN) {
            if (event.getX()>=(int)mV-5&&event.getX()<=mV+65&&event.getY()>=mV1-35&&event.getY()<=mV1+15){
            data2.data(mFormat1);
   ...
   getdata data2;
    public  interface  getdata{
        void data(String mFormat1);
    }
    public void setOnLetterChangedListener(getdata onLetterChangedListener) {
        this.data2 = onLetterChangedListener;
    }
